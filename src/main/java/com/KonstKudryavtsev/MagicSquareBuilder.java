...
public class MagicSquareBuilder {
    ...
    ** 5) Если попали на занятую ячейку, то возвращаемся на предыдущую и от нее на одну вниз.
     */
    ...
    ** 1) Выделяются области квадраты: углы размером n/4 * n/4 и центр размером n/2 на n/2.
    ** 2) Идем по ячейкам квадрата, начиная с верхней левой и заполняем (начиная с 1) только цифрами только выделенные
    **      в п.1 области, счет при этом не прекращая даже во время прохода незаполняемых ячеек.
    ** 3) Теперь идем по квадрату, начиная счет снова с 1 и с нижнего правого угла. Заполняем по тому же
    **      принципу, только теперь игнорируя ранее заполненные ячейки.
    */
